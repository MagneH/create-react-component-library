...
// Tests
it("should clone master branch by default", async (done) => {
  expect.assertions(7);
  ...
  const boilerplateGitRepo =
    "https://github.com/magneh/react-component-library.git";
...
it("should reject invalid project names", async (done) => {
   ...
   */
  await expect(
    init({ appDir, name: "invalidProjectName" })
  ).rejects.toThrowError("Project name can no longer contain capital letters");
  await expect(
    init({ appDir, name: "invalid project name" })
  ).rejects.toThrowError(
    "Project name can only contain URL-friendly characters"
  );
  await expect(
    init({ appDir, name: "_invalid-project-name" })
  ).rejects.toThrowError("Project name cannot start with an underscore");
  await expect(init({ appDir, name: "" })).rejects.toThrowError(
    "Project name length must be greater than zero"
  );
...
it("should give useful feedback if git clone fails", async (done) => {
   ...
   */
  await expect(init({ appDir, name })).rejects.toThrowError(
    "Failed to clone from git repo https://github.com/magneh/react-component-library.git. Make sure you have git (https://git-scm.com/) installed, the remote repository exists, you have the necessary permissions and internet connectivity."
  );
